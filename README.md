...
40. BOS里如何生成方法的同时生成注释?

> 答: 窗口 --> 首选项 --> Java --> 代码样式 --> 代码模板 --> 勾选[自动为新方法和类型添加注释]

41. BOS里如何重启服务器?

> 答: 点击右上角[调试透视图] --> 点击左上角[调试] --> 点击服务器 --> 点击页头的红色按钮[终止]即可
> 备注: 将服务器与客户端同时终止再进入建模工具里启动客户端，则服务器与客户端都同时启动了，
> 则相当于重启了客户端与服务器

42. 如何所有断点以及跳过所有断点?

> 答: 调试透视图 --> 断点 --> 除去所有断点/跳过所有断点

43. 如何找到自己已建好的F7所关联的是哪个info？

> 答: 设计视图 --> query里面 --> 子对象 --> 双击在子对象行的最后一个点
> 示例: com.kingdee.eas.mm.project.app.Project 为 ProjectInfo

44. 如何查找到自己已建好的F7是存在哪张表中的？

> 答: 设计视图 --> 点击query --> SQL --> 在里面找

45. 如何调整BOS工具选中文字其它相同文字也高亮显示?

> 答: 窗口 --> 首选项 --> 常规 --> 编辑器 --> 注释 --> 出现次数 --> 调整对应颜色而后应用即可
> 备注:调试当前指令为debug行的背景颜色

46. 如何根据表去反查它是什么基础资料？

> 答: 搜索 --> 搜索元数据 --> 输入表名 --> 在结果里双击进入即可看到别名就是

> 47. DEP的路径?

> 答: 企业建模 --> 业务对象建模 --> 动态扩展平台 --> 动态扩展平台高级版

> 48. 如何查看list中数据是存在哪张表的?

> 答： 选中某一行 ctrl + shift + c 得到一个uuid 到 查询分析器 Uuid查表名 中去
> 粘贴这个值点击确定即可看到所需要的表名
